...
# Histograma da Vari√°vel metano
x11()
hist(dados$ch4, main="xch4(ppb)",nclass=7,freq=F,xlab="Classes",   # slide 5
     ylab="Densidade",col="gray")
x11()
plot(ecdf(dados$ch4),main="FunÁ„o Densidade Acumulada Empirica", # slide 7
     xlab="Silte(%)",ylab="Probabilidade",ylim=c(0,1))

x11()       
qqnorm(dados$ch4,main="Gr√°fico Q-Q normal (xch4)",xlab="Quantis te√≥ricos", 
...
mk.test(dados1$ch4)
# Teste de Pettit da S√©rie Temporal
pettitt.test (dados1$ch4)
#########################################################
# Teste MAnn KenKall e Pettit m√©dia anual
mk.test(dados2$ch4)
# Teste de Pettit da S√©rie Temporal
